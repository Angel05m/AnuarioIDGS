...
<head>
    ...
    tailwind.config = {
      theme: {
        extend: {
          colors: {
            utesc: {
              dark: '#0D2A3F',   // azul institucional
              dark2:'#0A2536',   // azul más profundo
              teal: '#009B8C',   // verde turquesa
              ink:  '#F8FAFC',
              muted:'#CBD5E1'
            }
          },
  ...
  <style>
    /* ===== Fondo azul institucional con gradiente sutil ===== */
    body{
    ...
    /* Panel “glass” oscuro con borde turquesa */
    .glass{
    ...
    /* Botón primario turquesa (se ve más “botón”) */
    .btn-primary{
      background: #009B8C;
      color: #FFFFFF;
      border-radius: 9999px;
      border: 1px solid rgba(255,255,255,.45);   /* <-- borde */
      box-shadow: 0 10px 22px rgba(0,155,140,.32);
      transition: transform .15s ease, box-shadow .2s ease, background-color .2s ease, color .2s ease;
    }
    .btn-primary:hover{
      background:#007F76;
      box-shadow:0 12px 26px rgba(0,155,140,.4);
      transform: translateY(-1px);
    }

    /* Inputs oscuros con focus turquesa */
    .input-dark{
      background: rgba(13,42,63,.65);
    ...
    .input-dark:focus{
      outline: none;
      border-color: #009B8C;
      box-shadow: 0 0 0 3px rgba(0,155,140,.25);
    }
    ...
    <div class="max-w-7xl mx-auto px-2 py-4 flex items-center justify-between gap-4">
      ...
      <a href="{{ route('publications.index') }}" class="flex items-center gap-2 group pl-10 relative">
        ...
        <button type="button"
                onclick="appGoBack()"
                aria-label="Volver"
                class="absolute -left-8 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/20 hover:bg-white/30
                       ring-1 ring-white/30 grid place-items-center transition">
      ...
      <!-- Acciones de página (espacio opcional para vistas que inyecten controles) -->
      ...
      <div class="flex items-center gap-3">
        @unless (request()->routeIs('publications.create', 'publications.edit'))
          <a href="{{ route('publications.create') }}"
             class="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 btn-primary">
        ...
        <div class="relative" data-dropdown>
          ...
          <!-- Menú -->
          <div id="userMenu"
               class="invisible opacity-0 pointer-events-none absolute right-0 mt-2 w-80
                      rounded-2xl bg-white text-utesc-dark shadow-2xl ring-1 ring-black/5
            ...
            <div class="px-5 py-3 border-t border-slate-200">
              <a href="{{ url('/login') }}"
                 class="flex items-center gap-3 px-4 py-2 rounded-lg bg-rose-600 text-white font-semibold
                        hover:bg-rose-700 w-full justify-center">
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
  ...
  <script>
    ...
    (function(){
      ...
      btn.addEventListener('click', (e)=> {
        e.stopPropagation();
        (menu.classList.contains('invisible') ? open : close)();
      });
